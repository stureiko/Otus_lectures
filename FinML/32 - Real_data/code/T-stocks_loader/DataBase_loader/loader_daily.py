...
async def store_candle_data(conn, candle, figi, ticker):
    ...
    try:
        await conn.execute(
            f"""
            INSERT INTO daily_shares.{ticker} (figi,  time, open, close, high, low, volume)
...
async def download_tickers(figi, ticker_name):
    ...
    async with AsyncClient(TOKEN) as client:
        ...
        await conn.execute(f"""
            CREATE TABLE IF NOT EXISTS daily_shares.{ticker_name} (
                figi TEXT NOT NULL,
...
async def main():
    ...
    with engine.connect() as conn:
        figies = pd.read_sql('SELECT DISTINCT figi, ticker from metadata.stocks_info', conn)
