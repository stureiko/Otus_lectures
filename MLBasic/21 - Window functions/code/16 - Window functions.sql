...
-- Пронумеруем сотрудников внутри отделов по убыванию зп
SELECT 
...
-- Агрегация
-- посмотрим на сумму зарплат по департаментам и посчитаем  % для каждого сотрудника
SELECT
  name, department, salary,
...
-- посчитаем % зарплаты от всего фонда оплаты
SELECT
  name, department, salary,
  sum(salary) OVER w AS fund,
  round(salary * 100.0 / sum(salary) OVER w) || ' %' AS perc
