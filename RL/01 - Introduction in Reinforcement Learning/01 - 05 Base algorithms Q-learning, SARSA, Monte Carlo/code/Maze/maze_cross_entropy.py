...
def main():
    ...
    for i in range(episode_n):
        ...
        # get mean reward for this learn episode
        mean_total_reward = np.mean([trajectory['total_reward'] for trajectory in trajectories])

        # get elite trajectories
        elite_trajectories = get_elite_trajectories(trajectories, q_param)

        # update policy for elite trajectories
        if len(elite_trajectories) > 0:
            agent.update_policy(elite_trajectories)

        # add mean episode reward to array for plotting
        mean_episode_reward.append(mean_total_reward)

        print(f'Episode {i + 1} mean total reward for episode: {mean_total_reward:2.4f}')

    plt.plot(mean_episode_reward)
    plt.title('Cross-entropy algorithm. Dynamics of learning.')
    plt.xlabel('Episode')
