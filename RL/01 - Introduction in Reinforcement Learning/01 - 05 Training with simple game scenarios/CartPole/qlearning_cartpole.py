...
class QLearning:
    def __init__(self, env, epsilon=0.2, learning_rate=0.25, discount=0.95):
        ...
        self.theta_minmax = env.observation_space.high[2]
        self.theta_dot_minmax = math.radians(50)
        self.theta_state_size = 50
        self.theta_dot_state_size = 50
        self.q_table = np.random.randn(self.theta_state_size, self.theta_dot_state_size, env.action_space.n)

    def discretised_state(self, state):
        # Преобразование непрерывного состояния в дискретное состояние для использования в Q-таблице
    ...
    def get_action(self, discrete_state):
        # Выбор действия с вероятностью epsilon случайного действия
    ...
    def update_q_table(self, state, action, reward, next_state):
        # Обновление Q-таблицы в соответствии с алгоритмом Q-Learning
    ...
    def train(self, episodes, episode_display=500):
        ...
        for episode in range(episodes):
            ...
            while not done:
                ...
                if not done:
                    self.update_q_table(curr_discrete_state, curr_action, reward, new_discrete_state)
...
def main():
    ...
    q_learning_agent = QLearning(env, epsilon=EPSILON, learning_rate=LEARNING_RATE)

    q_learning_agent.train(EPISODES, EPISODE_DISPLAY)
