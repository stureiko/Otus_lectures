...
from buffer_test import replayBuffer
...
class MADDPG_Actor(nn.Module):
    def __init__(self, obs_size, n_actions):
        ...
        # Финальный выход нейронной сети обрабатывается функцией Tanh()
...
def select_actionFox(act_prob, avail_actions_ind, n_actions, noise_rate):
    ...
    act_prob = np.array([x if x > 0 else 0 for x in act_prob])
...
def main():
    ...
    experience_buffer = replayBuffer(buffer_size=buffer_len)
    ...
    for e in range(n_episodes):
        ...
        while not terminated:
            ...
            for agent_id in range(n_agents):
                ...
                obs_agentT = torch.FloatTensor(np.array([obs_agent[agent_id]])).to(device)
            ...
            if not terminated:
                for agent_id in range(n_agents):
                    ...
                    # Конвертируем данные в тензор
                    obs_agent_nextT = torch.FloatTensor(np.array([obs_agent_next[agent_id]])).to(device)
            ...
            experience_buffer.add([observations, actions, observations_next, actions_next, reward, terminated])
            ...
            if (global_step % steps_train == 0) and (global_step > start_steps):
                ...
                exp_obs, exp_acts, exp_next_obs, exp_next_acts, exp_rew, exp_termd = experience_buffer.sample(batch_size=batch_size)
                ...
                action_probabilityQT = critic_network(obs_agentsT, act_agentsT)
                action_probabilityQT = action_probabilityQT.to("cpu")
                ...
                action_probabilityQ_nextT = tgtCritic_network(obs_agents_nextT, act_agents_nextT)
                action_probabilityQ_nextT = action_probabilityQ_nextT.to("cpu")
                action_probabilityQ_next = action_probabilityQ_nextT.data.numpy()[0]

                # Переформатируем y_batch размером batch_size
                y_batch = np.zeros([batch_size])
                action_probabilityQBT = torch.empty(1, batch_size, dtype=torch.float)

                for i in range(batch_size):
                    # Вычисляем целевое значение y
                    y_batch[i] = exp_rew[i] + (gamma * action_probabilityQ_next[i]) * (1 - exp_termd[i])
                    action_probabilityQBT[0][i] = action_probabilityQT[0][i]
                ...
                loss_t_critic = objectiveCritic(action_probabilityQBT, y_batchT)
                ...
                # Усредняем значение по количеству элементов мини выборки
                ...
                # Реализуем механизм мягкой замены
        ...
        episode_duration.append(global_step)
